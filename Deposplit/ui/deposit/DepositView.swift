...
struct DepositFormContent<LeadingToolbar: ToolbarContent>: View {
    ...
    var body: some View {
        // Splitting needs k of n with k at least 2, so fewer than two contacts makes the whole form
        // moot. Offering it and disabling Deposit afterwards is a refusal nothing on screen predicted.
        // Repair reaches this too, and for the same reason: a re-split still needs holders.
        Group {
            if viewModel.allContacts.count < 2 {
                ContentUnavailableView(
                    "Not enough contacts",
                    systemImage: "person.2",
                    description: Text(
                        "At least two contacts are needed first — a secret is split among at least two people"
                    )
                )
            } else {
                Form {
        ...
        .toolbar {
            leadingToolbar()
            // Gone rather than disabled below the threshold, so the toolbar agrees with the body:
            // there is no form to confirm.
            if viewModel.allContacts.count >= 2 {
