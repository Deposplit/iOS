...
**Resolved 2026-08-24** (had been flagged above as a known limitation): the retrieve-approval "key changed N days ago" indicator no longer passes an English pluralization suffix (`"s"`/`""`) as a raw `%@` argument. `RecipientRequestsTab.swift`'s `Label(...)` now interpolates only `senderName` and `days`; `Localizable.xcstrings` carries a real `variations.plural` rule per language on the resulting key (`"%@'s key changed %lld days ago — verify fresh before approving"`) — English `one`/`other` as "day"/"days", German as "Tag"/"Tagen" — so each language supplies its own grammatically correct form instead of the call site hardcoding English's. Same pass also cleared the five stale `Localizable.xcstrings` entries Xcode had flagged (four genuinely dead leftovers from earlier renames/fixes, deleted outright; `"Split & Share"` turned out still live but silently un-localized by a `title: String` vs. `LocalizedStringKey` typing bug in `DepositFormContent` — fixed, and the identical bug at `RepairView`'s `"Repair — Re-deposit"` call site was caught and localized for the first time in the same pass). Verified: `xcodebuild build` succeeds, `swift test` 110/110.
